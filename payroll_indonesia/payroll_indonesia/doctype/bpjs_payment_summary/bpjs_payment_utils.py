...
def fetch_from_salary_slip(summary):
    ...
    try:
        ...
        if not summary.is_new():
            summary.save()
            frappe.msgprint(_("Successfully fetched BPJS data from {0} salary slips").format(len(summary.summary_details)))
...
def create_payment_entry(summary):
    ...
    try:
        ...
        if summary.docstatus != 1:
            frappe.throw(_("BPJS Payment Summary must be submitted before creating a Payment Entry"))
        
        # Check if summary has details
        if not summary.summary_details:
            frappe.throw(_("No BPJS payment details found. Please fetch data from salary slips first."))
