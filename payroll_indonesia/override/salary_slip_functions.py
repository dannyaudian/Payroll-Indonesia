...
def ensure_employee_tax_summary_integration(doc):
    ...
    try:
        ...
        tax_summary = frappe.get_all(
            ...
            fields=[
                "ytd_gross_pay", "ytd_tax", "ytd_bpjs", 
                "ytd_taxable_components", "ytd_tax_deductions",
                "tax_method", "is_using_ter", "ter_rate"
            ],
            limit=1
        )
        ...
        if hasattr(slip, "earnings") and slip.earnings:
            for earning in slip.earnings:
                if earning.salary_component == component_name:
                    ...
                    result["tax_effect"] = frappe.db.get_value("Salary Component", component_name, "tax_effect_type")
                    return result
